...
if __name__ == "__main__":
    ...
    crf1 = sklearn_crfsuite.CRF(algorithm='l2sgd', c2=0.23, max_iterations=800, period=12, delta=6.352631578947369e-05, calibration_eta=0.04268421052631579, calibration_rate=3.0, calibration_samples=1400, calibration_candidates=5, calibration_max_trials=10)
    crf1.fit(X1_train, y1_train)
    y1_pred = crf1.predict(X1_test)
    for sub_list in y1_pred:
        predict += sub_list

    crf2 = sklearn_crfsuite.CRF(algorithm='l2sgd', c2=0.34, max_iterations=1200, period=10, delta=6.873684210526315e-05, calibration_eta=0.07915789473684211, calibration_rate=5.0, calibration_samples=1400, calibration_candidates=20, calibration_max_trials=25)
    crf2.fit(X2_train, y2_train)
    y2_pred = crf2.predict(X2_test)
    for sub_list in y2_pred:
        predict += sub_list

    crf3 = sklearn_crfsuite.CRF(algorithm='l2sgd', c2=0.12, max_iterations=1100, period=12, delta=3.226315789473684e-05, calibration_eta=0.05831578947368421, calibration_rate=3.0, calibration_samples=800, calibration_candidates=25, calibration_max_trials=10)
    crf3.fit(X3_train, y3_train)
    y3_pred = crf3.predict(X3_test)
    for sub_list in y3_pred:
        predict += sub_list

    crf4 = sklearn_crfsuite.CRF(algorithm='l2sgd', c2=0.23, max_iterations=800, period=12, delta=6.352631578947369e-05, calibration_eta=0.04268421052631579, calibration_rate=3.0, calibration_samples=1400, calibration_candidates=5, calibration_max_trials=10)
    crf4.fit(X4_train, y4_train)
    y4_pred = crf4.predict(X4_test)
    for sub_list in y4_pred:
        predict += sub_list

    crf5 = sklearn_crfsuite.CRF(algorithm='l2sgd', c2=0.12, max_iterations=1100, period=12, delta=3.226315789473684e-05, calibration_eta=0.05831578947368421, calibration_rate=3.0, calibration_samples=800, calibration_candidates=25, calibration_max_trials=10)
